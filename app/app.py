...
def preprocess(data, scaler, feature_names):
    num_feats = ['age', 'years_employed', 'annual_income', 'credit_score', 'credit_history_years',
                 'savings_assets', 'defaults_on_file', 'delinquencies_last_2yrs', 'derogatory_marks',
    ...
    for col in [f"occupation_status_{data['occupation']}", f"product_type_{data['product']}", f"loan_intent_{data['intent']}"]:
...
def main():
    ...
    with col1:
        ...
        occupation = st.selectbox("Occupation", ["Employed", "Self-Employed", "Student"])
        ...
        product = st.selectbox("Product Type", ["Personal Loan", "Credit Card", "Line of Credit"])
        intent = st.selectbox("Loan Purpose", ["Personal", "Education", "Home Improvement", "Medical", "Business", "Debt Consolidation"])
        ...
        predict = st.button("Predict Approval", type="primary", use_container_width=True)

    with col2:
        if predict:
            data = {
                'age': age, 'years_employed': years_employed, 'annual_income': annual_income,
                'credit_score': credit_score, 'credit_history_years': credit_history,
                'savings_assets': savings, 'defaults_on_file': defaults,
                'delinquencies_last_2yrs': delinquencies, 'derogatory_marks': derogatory,
                'loan_amount': loan_amount, 'interest_rate': interest_rate,
                'debt_to_income_ratio': dti, 'loan_to_income_ratio': lti,
                'payment_to_income_ratio': pti, 'occupation': occupation,
                'product': product, 'intent': intent
            }
            ...
            c1.metric("Credit Score", credit_score, "Good" if credit_score >= 700 else "Fair" if credit_score >= 650 else "Low")
